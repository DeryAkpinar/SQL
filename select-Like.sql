...
drop table calisanlar; --silemezsiniz der konsolda
--primary key parent olur forein key ise child olur
...
ON DELETE CASCADE --iliski kurulduysa parenten silerken child'denda silinmesini istiyorsak muhakkak eklenmeli
);
...
select isim from personel WHERE isim ~~* 'a%n'  --a ile baslayan n ile biten
...
select isim from personel where isim ~~* '%e%' and isim ~~ '%r%'  --icinde e ve r olanlari 
--or deseydik birtanesi bile olsa onlari getirir and diyince iki sarti da saglamali hem e hem r olmali

-- 2. harfi e olup diğer harflerinde y olan personeli listeleyiniz
select * from personel WHERE isim like '_e%y%'  --2.harfi e olan 
